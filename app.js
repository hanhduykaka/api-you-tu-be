...
app.get('/getVideoStream/:id?', function (req, res) {
    try {
        ...
        youtubedl.getInfo(url, options, function (err, info) {
            if (err) {
                res.end(JSON.stringify({ 'code': 400,"message": "Bad request", "data":null}));
            };
        ...
        res.end(JSON.stringify({ 'code': 404,"message": "Bad request", "data":null }));
    }

});

app.get('/getVideoFullInfo/:id?', function (req, res) {
    try {
        var data = fs.readFileSync("./du_lieu/san_pham.json");
        res.setHeader("Access-Control-Allow-Origin", "*");
        var videoId = req.query.id;
